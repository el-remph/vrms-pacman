...
sub process {
	...
	# Trailingcomma makes it easier to diff the results with previous
	# results, especially if the latter was hand-hacked or the fields
	# transposed change. If all else fails, comm -3 <(sort a) <(sort b)
	$_ = Data::Dumper->new([\%result])
		->Terse(1)->Indent(1)->Useqq(1)->Sortkeys(1)->Trailingcomma(1)
		->Dump;
	undef %result;
